...
import { PlusCircle, X, Loader2, CheckCircle2, AlertCircle, Trash2, Plus, Info } from 'lucide-react'
...
const ITEM_VACIO: ItemLocal = { producto_id: '', descripcion: '', cantidad: '1', precio_unitario: '', iva_porcentaje: '19' }

function hoy() {
  return new Date().toISOString().slice(0, 10)
}

export default function FormFacturaCompra({ proveedores, productos }: Props) {
  ...
  function seleccionarProducto(idx: number, productoId: string) {
    ...
    setItems(items.map((item, i) => {
      ...
      if (!producto) return { ...item, producto_id: '', iva_porcentaje: '19' }
      return {
        ...item,
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl my-8 shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">La compra actualiza automaticamente el costo promedio y el stock</p>
          </div>
        ...
        <form action={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input name="numero_factura" type="text" required placeholder="Ej: FV-001234" className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm" />
            </div>
          </div>
              ...
              {items.map((item, idx) => (
                <div key={idx} className="bg-gray-50 p-3 rounded-xl space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <div>
                      <label className="block text-xs text-gray-500 mb-0.5">P. Compra</label>
                      <input
          ...
          {/* Info banner */}
          <div className="flex items-start gap-2 p-3 rounded-xl bg-blue-50 border border-blue-200 text-sm text-blue-700">
            <Info className="w-4 h-4 mt-0.5 flex-shrink-0" />
            <span>Al guardar, el costo promedio y stock de cada producto se actualizan automaticamente</span>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={pendiente} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
              {pendiente && <Loader2 className="w-4 h-4 animate-spin" />} Registrar factura
            </button>
          </div>
        </form>
      </div>
