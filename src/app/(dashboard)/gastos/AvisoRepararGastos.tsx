...
export default function AvisoRepararGastos({ reparables, sinVenta }: Props) {
  ...
  const totalReparable = reparables.reduce((s, g) => s + g.monto, 0)

  function reparar() {
    startTransition(async () => {
      const res = await repararGastosSinVenta()
      setResultado(res)
        ...
        <div className="min-w-0 flex-1">
          ...
          {reparables.length > 0 && (
            <div className="mt-3 rounded-xl border border-amber-200 bg-white p-3">
              <p className="text-xs font-semibold text-gray-700">
                {reparables.length} gasto(s) por {formatCOP(totalReparable)} se pueden reparar solos:
              </p>
              <ul className="mt-1.5 space-y-1">
                {reparables.slice(0, 8).map((g) => (
                  <li key={g.id} className="flex items-center justify-between gap-3 text-xs text-gray-600">
                    <span className="truncate">
                      {g.concepto}
                      {g.cotizacion_numero && (
                        <span className="ml-1.5 font-medium text-gray-800">→ {g.cotizacion_numero}</span>
                      )}
                    </span>
                ...
                {reparables.length > 8 && (
                  <li className="text-xs text-gray-400">y {reparables.length - 8} mas...</li>
                )}
              </ul>
            </div>
          )}
          ...
          {reparables.length > 0 && (
            ...
            >
              ...
              {pendiente ? 'Reparando...' : 'Reparar ahora'}
            </button>
          )}
        </div>
      </div>
